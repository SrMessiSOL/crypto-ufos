...
const [tradeQuantity, setTradeQuantity] = useState<{ [key: string]: number }>(() =>
  ...
  const [showProgressBars, setShowProgressBars] = useState(true); // New state for progress bars visibility
...
const shouldShowProgressBars = showProgressBars && !isInventoryOpen && !isSkillPointsModalOpen && !isMarketOpen && !isLaboratoryOpen && !isFabricatorModalOpen && !isProfessionModalOpen && !isTransferFormOpen && !isNameFormOpen && !isConfirmModalOpen;
  ...
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      const safeData: Record<string, any> = {
        ...
        ScavengerWorkingEnd: updatedData.scavengerWorkingEnd,
        cadWorking: updatedData.cadWorking,
        cadWorkingEnd: updatedData.cadWorkingEnd,
        ...
        SelectedNFT: updatedData.selectedNFT || null,
        ActiveProfession: updatedData.activeProfession,
        Professions: updatedData.professions,
        CrystalOre: updatedData.crystalOre,
        RareEarths: updatedData.rareEarths,
        PurifiedWater: updatedData.purifiedWater,
        PlasmaFluid: updatedData.plasmaFluid,
        QuantumCells: updatedData.quantumCells,
        EnergyCores: updatedData.energyCores,
        Biofiber: updatedData.biofiber,
        SporeEssence: updatedData.sporeEssence,
        AlloyIngots: updatedData.alloyIngots,
        Nanosteel: updatedData.nanosteel,
        Catalysts: updatedData.catalysts,
        Polymers: updatedData.polymers,
        SpareParts: updatedData.spareParts,
        CircuitBoards: updatedData.circuitBoards,
        TradeContracts: updatedData.tradeContracts,
        MarketTokens: updatedData.marketTokens,
        ProcessedGems: updatedData.processedGems,
        ExoticCrystals: updatedData.exoticCrystals,
        HydrogenFuel: updatedData.hydrogenFuel,
        FusionFluid: updatedData.fusionFluid,
        PlasmaCores: updatedData.plasmaCores,
        AntimatterCells: updatedData.antimatterCells,
        BioPolymers: updatedData.bioPolymers,
        NanoOrganics: updatedData.nanoOrganics,
        SuperAlloys: updatedData.superAlloys,
        MetaMaterials: updatedData.metaMaterials,
        NanoCatalysts: updatedData.nanoCatalysts,
        QuantumChemicals: updatedData.quantumChemicals,
        AdvancedComponents: updatedData.advancedComponents,
        RoboticModules: updatedData.roboticModules,
        CryptoCredits: updatedData.cryptoCredits,
        GalacticBonds: updatedData.galacticBonds,
        SolarPanel: updatedData.solarPanel,
        IonThruster: updatedData.ionThruster,
        LifeSupportModule: updatedData.lifeSupportModule,
        QuantumDrive: updatedData.quantumDrive,
        NanoAssembler: updatedData.nanoAssembler,
        BioCircuit: updatedData.bioCircuit,
        CrystalMatrix: updatedData.crystalMatrix,
        HydroCore: updatedData.hydroCore,
        TradeBeacon: updatedData.tradeBeacon,
        GravitonShield: updatedData.gravitonShield,
        NeuralInterface: updatedData.neuralInterface,
        AntimatterWarhead: updatedData.antimatterWarhead,
        HoloProjector: updatedData.holoProjector,
        BioReactorCore: updatedData.bioReactorCore,
        BuildingLevels: updatedData.buildingLevels,
        BuildingTimestamps: Object.keys(updatedData.buildingTimestamps).reduce(
          (acc, key) => ({
            ...acc,
            [key]: updatedData.buildingTimestamps[key] instanceof Date && !isNaN(updatedData.buildingTimestamps[key].getTime())
              ? updatedData.buildingTimestamps[key]
              : null,
          }),
          {}
        ),
        BuildingClaimables: updatedData.buildingClaimables,
        SelectedFabricatorProduct: updatedData.selectedFabricatorProduct,
        StreakCount: updatedData.streakCount,
        ClaimableFabricatorProduct: updatedData.claimableFabricatorProduct ?? null,
      };

      // Handle timestamps
      safeData.ProfessionSwitchTimestamp = updatedData.professionSwitchTimestamp instanceof Date &&
        !isNaN(updatedData.professionSwitchTimestamp.getTime())
        ? updatedData.professionSwitchTimestamp
        : null;
      safeData.TimeStamp = updatedData.timeStamp instanceof Date && !isNaN(updatedData.timeStamp.getTime())
        ? updatedData.timeStamp
        : null;
      safeData.TimeStampScavenger = updatedData.timeStampScavenger instanceof Date &&
        !isNaN(updatedData.timeStampScavenger.getTime())
        ? updatedData.timeStampScavenger
        : null;
      safeData.TimeStampCad = updatedData.timeStampCad instanceof Date && !isNaN(updatedData.timeStampCad.getTime())
        ? updatedData.timeStampCad
        : null;
      safeData.TimeStampW = updatedData.timeStampW instanceof Date && !isNaN(updatedData.timeStampW.getTime())
        ? updatedData.timeStampW
        : null;
      safeData.TimeStampS = updatedData.timeStampS instanceof Date && !isNaN(updatedData.timeStampS.getTime())
        ? updatedData.timeStampS
        : null;
      safeData.TimeStampDailyClaim = updatedData.timeStampDailyClaim instanceof Date &&
        !isNaN(updatedData.timeStampDailyClaim.getTime())
        ? updatedData.timeStampDailyClaim
        : null;

      safeData.PowerCellSlots = updatedData.powerCellSlots?.map((slot) => ({
        ...slot,
        timeStamp: slot.timeStamp instanceof Date && !isNaN(slot.timeStamp.getTime()) ? slot.timeStamp : null,
      })) || [];

      await updateDoc(docRef, safeData);
      localStorage.setItem(`userData_${walletAddress}`, JSON.stringify(updatedData));
      console.log("[saveUserData] Data saved successfully for wallet:", walletAddress);
    } else {
      console.warn("[saveUserData] No document found for wallet:", walletAddress);
      showToast("error", "User Not Found", "No user document found. Please reconnect your wallet.", 3000);
    }
  } catch (error) {
...
const renderResourceToastContent = (message: string, items: { key: ResourceKey | "text"; amount: number; text?: string }[]) => {
  ...
  const getProgressItems = (
    ...
    if (userData.scavengerWorking > 0) {
      items.push({
        key: "scavenger",
        progress: progressBars.scavenger,
        cycleHours: 6,
    ...
    // CAD
    if (userData.cadWorking > 0) {
      items.push({
        key: "cad",
        progress: progressBars.cad,
        cycleHours: 6,
    ...
    if (userData.chargingWaterFilter > 0) {
      items.push({
        key: "waterFilter",
        progress: progressBars.waterFilter,
        cycleHours: 8,
    ...
    if (userData.chargingWorkShop > 0) {
      items.push({
        key: "workshop",
        progress: progressBars.workshop,
        cycleHours: 10,
    ...
    Object.keys(buildingConfig).forEach((building) => {
      if (userData.buildingTimestamps[building]) {
        ...
        // Special handling for Interstellar Fabricator
        if (building === "InterstellarFabricator" && userData.selectedFabricatorProduct) {
          ...
          if (fabricatorConfig) {
            ...
            outputKey = fabricatorConfig.outputKey; // Now typed as FabricatorOutputKey
          } else {
            console.warn(`[getProgressItems] Invalid fabricator product: ${userData.selectedFabricatorProduct}`);
            return; // Skip invalid product
          }
        } else {
          // For other buildings, use the first key of output
          outputKey = Object.keys(config.output)[0] as keyof typeof GAME_ASSETS;
          if (!outputKey || !(outputKey in GAME_ASSETS)) {
            console.warn(`[getProgressItems] No valid output key for ${building}, using building asset`);
        ...
        items.push({
          key: building,
          progress: progressBars[building] || 0,
...
const updateProgressBars = () => {
  const now = new Date();
  console.log(`[updateProgressBars] Current time: ${now.toISOString()}`);

  // Track if userData needs to be updated
  let userDataChanged = false;
  const updatedUserData = { ...userData };

  // Update power cell slots progress
  const updatedSlots = powerCellSlots.map((slot) => {
    // Check for invalid state: isCharging is true but timeStamp is null
    if (slot.isCharging && !slot.timeStamp) {
      console.warn(`[updateProgressBars] Fixing invalid state for slot ${slot.id}: isCharging=true, timeStamp=null`);
      userDataChanged = true;
      updatedUserData.claimableFullPowerCell = (updatedUserData.claimableFullPowerCell || 0) + 1;
      return {
        ...slot,
        isCharging: false,
        isClaimable: true,
  ...
  if (JSON.stringify(updatedSlots) !== JSON.stringify(powerCellSlots)) {
    setPowerCellSlots(updatedSlots);
    updatedUserData.powerCellSlots = updatedSlots;
  }

  // Check and fix invalid states for scavenger, cad, waterFilter, and workshop
  if (userData.scavengerWorking > 0 && !userData.timeStampScavenger) {
    console.warn(`[updateProgressBars] Fixing invalid state for scavenger: scavengerWorking=1, timeStampScavenger=null`);
    userDataChanged = true;
    updatedUserData.scavengerWorking = 0;
    updatedUserData.scavengerWorkingEnd = 1;
    updatedUserData.timeStampScavenger = null;
  }

  if (userData.cadWorking > 0 && !userData.timeStampCad) {
    console.warn(`[updateProgressBars] Fixing invalid state for cad: cadWorking=1, timeStampCad=null`);
    userDataChanged = true;
    updatedUserData.cadWorking = 0;
    updatedUserData.cadWorkingEnd = 1;
    updatedUserData.timeStampCad = null;
  }

  if (userData.chargingWaterFilter > 0 && !userData.timeStampW) {
    console.warn(`[updateProgressBars] Fixing invalid state for waterFilter: chargingWaterFilter=1, timeStampW=null`);
    userDataChanged = true;
    updatedUserData.chargingWaterFilter = 0;
    updatedUserData.claimableWater = 1;
    updatedUserData.timeStampW = null;
  }

  if (userData.chargingWorkShop > 0 && !userData.timeStampS) {
    console.warn(`[updateProgressBars] Fixing invalid state for workshop: chargingWorkShop=1, timeStampS=null`);
    userDataChanged = true;
    updatedUserData.chargingWorkShop = 0;
    updatedUserData.claimableEmptyPowerCell = 1;
    updatedUserData.timeStampS = null;
  }
  ...
  Object.keys(buildingConfig).forEach((building) => {
    ...
    if (timestamp instanceof Date && !isNaN(timestamp.getTime())) {
      ...
      if (building === "InterstellarFabricator" && userData.selectedFabricatorProduct) {
        ...
        if (config) {
          ...
          console.error(`[updateProgressBars] Invalid product for Interstellar Fabricator: ${userData.selectedFabricatorProduct}`);
          updatedUserData.buildingTimestamps[building] = null;
          updatedUserData.selectedFabricatorProduct = null;
          updatedUserData.claimableFabricatorProduct = null;
          userDataChanged = true;
      ...
      if (elapsed < 0) {
        console.error(`[updateProgressBars] Negative elapsed time for ${building}: ${elapsed}s`);
        progress = 0;
      } else if (elapsed >= total) {
        console.log(`[updateProgressBars] Production complete for ${building}`);
        ...
        updatedUserData.buildingTimestamps[building] = null;
        updatedUserData.buildingClaimables[building] = newClaimableCount;
        updatedUserData.claimableFabricatorProduct = newClaimableProduct;
        userDataChanged = true;

        if (building === "InterstellarFabricator" && !notifiedActions[building]) {
          setFabricatorProgress(100);
          setClaimableFabricatorProduct(newClaimableProduct);
          showToast("success", "Production Complete", `Interstellar Fabricator production is ready to claim!`, 3000);
          setIsFabricatorModalOpen(true);
          markActionNotified(building);
        } else if (!notifiedActions[building]) {
          showToast("success", "Production Complete", `${buildingConfig[building]?.name || building} production is ready to claim!`, 3000);
          markActionNotified(building);
        }
        progress = 100;
      } else {
        ...
        if (building === "InterstellarFabricator") {
          setFabricatorProgress(progress);
        }
      }
    } else if (userData.buildingTimestamps[building]) {
      console.warn(`[updateProgressBars] Invalid timestamp for ${building}: ${timestamp}`);
      progress = 0;
      if (building === "InterstellarFabricator") {
        setFabricatorProgress(0);
      }
    ...
    newProgressBars[building] = progress;
  });

  // Update userData only if necessary
  if (userDataChanged) {
    setUserData(updatedUserData);
    saveUserData(updatedUserData);
  }

  // Update progressBars only if changed
  if (JSON.stringify(newProgressBars) !== JSON.stringify(progressBars)) {
  ...
  // Original scavenger, water filter, workshop, cad logic
  ...
  const cadElapsed = userData.timeStampCad ? (now.getTime() - userData.timeStampCad.getTime()) / 1000 : 0;
  const cadTotal = 12 * 60 * 60;
  const cadProgress = Math.min(100, (cadElapsed / cadTotal) * 100);

  // Update progressBars only if values changed
  if (
    scavengerProgress !== progressBars.scavenger ||
    waterFilterProgress !== progressBars.waterFilter ||
    workshopProgress !== progressBars.workshop ||
    cadProgress !== progressBars.cad
  ) {
    setProgressBars((prev) => ({
      ...prev,
  ...
  if (cadProgress >= 100 && userData.cadWorking > 0) {
    handleCadComplete();
  }
};
...
showToast("error", "Wallet Not Connected", "Please connect your wallet to play the game.", 3000);
  ...
  try {
    ...
    // Calculate total power cells to claim
    ...
    // Award 32 EXP per power cell claimed
    if (userData.selectedNFT) {
      await addNFTExperience(32 * totalCells); // Wait for Firestore transaction
    }
...
const startBuildingProduction = (building: string, product?: string) => {
  if (!walletConnected) {
showToast("error", "Wallet Not Connected", "Connect your wallet first.", 3000);    return;
  }
...
showToast("error", "Wrong Profession", `Only a ${requiredProfession} can operate the ${buildingConfig[building]?.name || building}.`, 3000);
  ...
  if (userData.buildingTimestamps[building]) {
showToast("error", "Production in Progress", `${buildingConfig[building]?.name || building} is already producing. Please wait until the current production is complete or claim the output.`, 3000);
    return;
  }
...
const claimBuildingOutput = (building: string) => {
  if (!walletConnected) {
showToast("error", "Wallet Not Connected", "Connect your wallet first.", 3000);    return;
  }
...
showToast("error", "Nothing to Claim", `${buildingConfig[building]?.name || building} hasn't completed production.`, 3000);
  ...
  if (building === "InterstellarFabricator") {
    console.log(`[claimBuildingOutput] Attempting to claim product: ${selectedProductKey}`);
    if (!selectedProductKey || !fabricatorProducts[selectedProductKey]) {
      ...
      selectedProductKey = Object.keys(fabricatorProducts)[0]; // Fallback
showToast("warning", "Product Issue", "No valid product selected. Using default product for claiming.", 3000);
    ...
    console.log(`[claimBuildingOutput] Using product config: ${config.name}`);
  }
...
showToast("error", "Invalid Building/Product", `${buildingDisplayName} or product is not recognized.`, 3000);
  ...
  // Award 100 EXP for claiming building output
  if (userData.selectedNFT) {
    addNFTExperience(128);
  }
  ...
  saveUserData(updatedUserData);
  if (building === "InterstellarFabricator") {
    setFabricatorProgress(0); // Reset progress after claiming
  }
  setIsClaiming(false);
...
showToast("error", "Not Enough Full Power Cells", "You need at least one full power cell to start the ice miner.", 3000);
    ...
    if (userData.scavengerWorking > 0) {
showToast("error", "Already Mining", "Your ice miner is already working.", 3000);
      return
    }
    ...
    setUserData(updatedUserData)
    saveUserData(updatedUserData)
      clearActionNotification("scavenger"); // Clear notification status
...
showToast("error", "Wallet Not Connected", "Please connect your wallet to play the game.", 3000);
    ...
    if (userData.cadWorking > 0) {
showToast("error", "Already Mining", "Your C.A.D. is already working.", 3000);
      return
    }
...
showToast("error", "Not Enough Full Power Cells", "You need one full power cell to start the C.A.D.", 3000);
    ...
    setUserData(updatedUserData)
    saveUserData(updatedUserData)
  clearActionNotification("cad"); // Clear notification status
...
const claimScavengerResults = () => {
  ...
  // Check if scavenger is still working and time hasn't elapsed
  const now = new Date();
  const scavengerStart = userData.timeStampScavenger;
  if (userData.scavengerWorking > 0 && scavengerStart) {
    const elapsedSeconds = (now.getTime() - scavengerStart.getTime()) / 1000;
  ...
  // Award resources only when claimed
  const iceFound = Math.floor(Math.random() * 200) + 200;
  const iceFoundBonus = Math.floor(iceFound * (1 + totalBonus) * 100) / 100; // Round to 2 decimals
  const ufosFound = Math.floor(Math.random() * 90) + 100;
  const haliteGained = Math.floor(Math.random() * 2) + 2;
  const haliteGainedBonus = Math.floor(haliteGained * (1 + totalBonus) * 100) / 100; // Round to 2 decimals
  ...
  const claimCadResults = () => {
    ...
    // Check if scavenger is still working and time hasn't elapsed
    const now = new Date();
    const cadStart = userData.timeStampCad;
    if (userData.cadWorking > 0 && cadStart) {
      const elapsedSeconds = (now.getTime() - cadStart.getTime()) / 1000;
      const totalSeconds = 6 * 60 * 60; // 6 hours
      if (elapsedSeconds < totalSeconds) {
showToast("error", "Not Ready Yet", "The C.A.D. is still working. Please wait until it completes.", 3000);
        return;
      }
    }
...
showToast("error", "Wallet Not Connected", "Please connect your wallet to play the game.", 3000);
    ...
    if (userData.chargingWaterFilter > 0) {
showToast("error", "Already Filtering", "Your water filter is already working.", 3000);
      return
    } else {

    if (userData.fullPowerCell < 1 || userData.ice < 1000) {
      showToast("error", "Not enough resources", "You need 1 full power cell and 1000 ice to start the water filter.", 3000);
    ...
    setUserData(updatedUserData)
    saveUserData(updatedUserData)
  clearActionNotification("waterFilter"); // Clear notification status
...
showToast("error", "Wallet Not Connected", "Please connect your wallet to play the game.", 3000);
  ...
  // Determine brokenPowerCellFound first
  const brokenPowerCellGained = Math.random() < 0.5 ? 1 : 0;
  // Set emptyPowerCellFound to 0 if brokenPowerCellFound is 1, otherwise use random chance
...
showToast("error", "Insufficient Resources", "You need 1 full power cell and 1000 ice to start the water filter.", 3000);
    ...
    if (userData.chargingWorkShop > 0) {
showToast("error", "Nothing to Claim", "Your workshop hasn't completed its work yet.", 3000);      return
    }
    ...
    setUserData(updatedUserData)
    saveUserData(updatedUserData)
  clearActionNotification("workshop"); // Clear notification status
...
useEffect(() => {
  const interval = setInterval(() => {
    setProgressBars((prevProgressBars) => {
      ...
      newProgressBars.powerCell =
        powerCellSlots.reduce((acc, slot) => acc + (slot.progress || 0), 0) / powerCellSlots.length || 0;
  ...
  return () => clearInterval(interval);
}, [userData, powerCellSlots, handleScavengerComplete, handleWaterFilterComplete, handleWorkshopComplete, handleCadComplete]);
...
console.log("Solana version:", version);
  ...
  const transferAmountLamports = 0.0001 * web3.LAMPORTS_PER_SOL; // 0.001 SOL in lamports
...
showToast("error", "Claim First", "Please claim the charged power cell first.", 3000);
    ...
    setUserData(updatedUserData)
    saveUserData(updatedUserData)
  clearActionNotification(`powerCellSlot_${slotId}`); // Clear notification status
  }
...
showToast("error", "Nothing to Claim", "No power cell slots are ready to claim.", 3000);
  ...
  try {
    ...
    const updatedSlots = powerCellSlots.map((s) =>
      s.id === slotId
        ? { ...s, isCharging: false, isClaimable: false, timeStamp: null, progress: 0 }
        : s
    );
    ...
    // Award 32 EXP for claiming power cell
    if (userData.selectedNFT) {
      await addNFTExperience(32); // Wait for Firestore transaction
    }
...
<div className="fixed bottom-16 left-1/2 transform -translate-x-1/2 z-[300] bg-black/70 rounded-md p-2 max-h-[30vh] overflow-y-auto">
  <div className="flex justify-between items-center mb-2">
    ...
    <Button
      ...
      aria-label={showProgressBars ? "Hide progress bars" : "Show progress bars"}
    >
      {showProgressBars ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
    </Button>
  </div>
  {shouldShowProgressBars && (
    <div className="space-y-1">
      {getProgressItems(userData, progressBars, powerCellSlots, buildingConfig, buildingAssetMap).map((item) => (
      ...
      <CardContent className="p-4 flex-grow overflow-y-auto">
            ...
            <div key={slot.id} className="relative">
              ...
              {slot.isCharging && (
                <div className="mt-1">
                  ...
                  <p className="cypherpunk-time mt-1">{formatTimeRemaining(slot.progress, 12)}</p>
                </div>
              )}
        ...
        {isActionLocked && (
          ...
          {isFabricatorModalOpen && (
            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[999]">
              <Card className="w-[95vw] max-w-[500px] bg-gray-900 border-white text-white max-h-[80vh] flex flex-col z-[999]">
                <CardContent className="p-4 flex-grow overflow-y-auto">
                  <div className="flex justify-between items-center mb-4 sticky top-0 bg-gray-900 z-[999]">
                    <h2 className="text-base font-bold text-purple-400">Interstellar Fabricator</h2>
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setIsFabricatorModalOpen(false)}
                      className="text-white cursor-pointer"
                      aria-label="Close Interstellar Fabricator modal"
                    >
                      X
                    </Button>
                  </div>
                  {userData?.buildingClaimables.InterstellarFabricator > 0 ? (
                    <div className="mb-4 text-center">
                      ...
                      <Button
                        ...
                        disabled={isClaiming}
                        aria-label="Claim Interstellar Fabricator output"
                      >
                        {isClaiming
                          ? "Claiming..."
                    ...
                    <div className="mb-4 text-center">
                      ...
                      <Progress value={fabricatorProgress} className="h-2 w-full [&>div]:bg-green-500 mt-2" />
                    ...
                    <div className="space-y-4">
                      <p className="text-xs text-gray-400">Select a product to begin production:</p>
                      {Object.entries(fabricatorProducts).map(([productKey, product]) => {
                        ...
                        return (
                          <div key={productKey} className="p-3 border border-gray-700 rounded-md">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                />
                                <div>
                                  <h3 className="text-base font-medium text-purple-500">{product.name}</h3>
                                  <p className="text-xs text-gray-400">Cycle: {product.cycleHours} hours</p>
                                </div>
                              </div>
                              <Button
                                onClick={() => startBuildingProduction("InterstellarFabricator", productKey)}
                                disabled={!canProduce}
                                className={canProduce ? "bg-blue-500 hover:bg-blue-600 text-sm" : "bg-gray-500 text-sm"}
                                aria-label={`Produce ${product.name}`}
                              >
                                Produce
                              </Button>
                            </div>
                            <div className="mt-2">
                              <p className="text-xs text-yellow-400">Requirements:</p>
...
{/* Zoom Controls and Progress Bar Toggle */}
       ...
       <Button
        ...
        >
          ...
          <span className="text-sm">{showProgressBars ? "Hide Progress Bars" : "Show Progress Bars"}</span>
        </Button>
      </div>
          {isSkillPointsModalOpen && userData.selectedNFT && (
          <div
...
{shouldShowProgressBars && (
  <div className="fixed right-125 z-40">
    {getProgressItems(userData, progressBars, powerCellSlots, buildingConfig, buildingAssetMap).map((item, index) => (
      <div
        key={item.key}
