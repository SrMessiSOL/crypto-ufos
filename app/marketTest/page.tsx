...
const handleCloseModal = () => {
  ...
  const calculateTotalUfosTraded = (logs: MarketOrderLog[]) => {
    const total = logs.reduce((sum, log) => {
      ...
      console.log(`Log ${log.logId}: totalUfos = ${ufos}, Resource = ${log.resource}`);
      return sum + ufos;
    }, 0);
  ...
  try {
    await runTransaction(firestore, async (transaction) => {
      ...
      try {
        ...
        const buyerData: UserData = mapFirestoreToUserData(buyerDoc.data, buyerWallet);
        const sellerData: UserData = mapFirestoreToUserData(sellerDoc.data, sellerWallet);
        ...
        if (order.type === "sell") {
          // Buyer is purchasing, check UFOS balance
          ...
          // Buyer is selling, check resource balance
          const buyerResourceAmount = Number(buyerData[order.resource]) || 0;
          if (buyerResourceAmount < tradeAmount) {
            if (buyerResourceAmount <= 0) {
              throw new Error(`Insufficient ${order.resource}: You have ${buyerResourceAmount}, need ${tradeAmount}`);
            }
            tradeAmount = buyerResourceAmount;
            ufosTransferred = tradeAmount * order.pricePerUnit;
          }
        }
        ...
        // Update wallet balances (no deductions, as assets are in escrow)
        const updatedBuyerData: UserData = {
          ...buyerData,
          [order.resource]: (Number(buyerData[order.resource]) || 0) + tradeAmount,
        };
        const updatedSellerData: UserData = {
          ...sellerData,
          ufos: sellerData.ufos + ufosTransferred,
        };
...
const createBuyOrder = async () => {
  ...
  try {
    await runTransaction(firestore, async (transaction) => {
      ...
      const buyerUfos = Number(buyerData.UFOS) || 0;
      if (buyerUfos < totalCost) {
        throw new Error(`Insufficient UFOS: You have ${buyerUfos}, need ${totalCost}`);
      }

      // Deduct UFOS for the entire order amount (escrow)
      let updatedBuyerData: UserData = {
        ...mapFirestoreToUserData(buyerData, currentWalletAddress),
        ufos: buyerUfos - totalCost,
      };

      // Prepare initial wallet update
      const walletUpdates: Array<{ ref: any; data: UserData }> = [
        { ref: buyerDoc.ref, data: updatedBuyerData },
      ];

      // Attempt to match the order
      const { remainingAmount, matched, ufosSpent } = await matchOrder(
        {
          wallet: currentWalletAddress,
      ...
      // Refund excess UFOS if matched at a lower price
      if (matched) {
        updatedBuyerData = {
          ...updatedBuyerData,
          ufos: updatedBuyerData.ufos + (totalCost - ufosSpent),
        };
        walletUpdates[0] = { ref: buyerDoc.ref, data: updatedBuyerData }; // Update initial wallet update
      }
      ...
      if (remainingAmount > 0) {
        ...
        transaction.set(doc(firestore, "MarketOrders", orderId), newOrder);
        ...
        const log: MarketOrderLog = {
          ...
          sellerWallet: "",
          buyerBefore: { ufos: buyerUfos, resource: Number(buyerData[activeResource]) || 0 },
          buyerAfter: { ufos: updatedBuyerData.ufos, resource: Number(buyerData[activeResource]) || 0 },
...
const createSellOrder = async () => {
  ...
  try {
    ...
    const resourceAmountInState = Number(userData[activeResource]) || 0;
    if (resourceAmountInState < parsedSellAmount) {
      throw new Error(`Insufficient ${activeResource}: You have ${resourceAmountInState}, need ${parsedSellAmount}`);
    }
    ...
    await runTransaction(firestore, async (transaction: Transaction) => {
      ...
      const firestoreFieldName = getFirestoreFieldFromUserDataKey(activeResource);
      const resourceAmount = Number(sellerData[firestoreFieldName]) || 0;
      ...
      // Prepare seller data with resources deducted (escrow)
      let updatedUserData: UserData = {
        ...mapFirestoreToUserData(sellerData, currentWalletAddress),
        [activeResource]: resourceAmount - parsedSellAmount,
      };

      // Collect wallet updates
      const walletUpdates: Array<{ ref: any; data: UserData }> = [
        { ref: sellerDoc.ref, data: updatedUserData },
      ];
      ...
      if (matched) {
        updatedUserData = {
          ...updatedUserData,
          ufos: updatedUserData.ufos + ufosReceived,
        };
        walletUpdates[0] = { ref: sellerDoc.ref, data: updatedUserData }; // Update initial wallet update
      }
      ...
      if (remainingAmount > 0) {
        ...
        const log: MarketOrderLog = {
          ...
          sellerBefore: { ufos: updatedUserData.ufos, resource: resourceAmount },
          sellerAfter: { ufos: updatedUserData.ufos, resource: Number(updatedUserData[activeResource]) },
          timestamp: Timestamp.fromDate(new Date()),
          status: "active",
        };
        logs.push(log);
...
const matchOrder = async (
  ...
  let matched = false;
  let ufosSpent = 0; // For buy orders
  let ufosReceived = 0; // For sell orders
  const logs: MarketOrderLog[] = [];
  const walletUpdates: Array<{ ref: any; data: UserData }> = [];
  const orderUpdates: Array<{ ref: any; data: { status?: string; amount?: number } }> = [];

  // Fetch compatible orders
  ...
  const q = query(
    ...
    where("status", "==", "active"),
    orderBy("pricePerUnit", type === "buy" ? "asc" : "desc"), // Best price first
  ...
  for (const order of compatibleOrders) {
    ...
    const orderSnapshot = await transaction.get(orderRef);
    if (!orderSnapshot.exists() || orderSnapshot.data().status !== "active") {
      continue; // Skip if order is no longer active
    }

    const currentOrderAmount = Number(orderSnapshot.data().amount) || 0;
    if (currentOrderAmount <= 0) continue;

    // Determine trade amount
    const tradeAmount = Math.min(remainingAmount, currentOrderAmount);
    const matchPrice = order.pricePerUnit; // Use existing order's price
    const ufosTransferred = tradeAmount * matchPrice;

    // Track UFOS spent (buy orders) or received (sell orders)
    if (type === "buy") {
    ...
    // Prepare wallet updates (no deductions, as assets are in escrow)
    const updatedBuyerData: UserData = {
      ...buyerData,
      [resource]: (Number(buyerData[resource]) || 0) + tradeAmount,
    };
    const updatedSellerData: UserData = {
      ...sellerData,
      ufos: sellerData.ufos + ufosTransferred,
    ...
    const sellerAfter = { ufos: updatedSellerData.ufos, resource: Number(sellerData[resource]) || 0 };
  ...
  try {
    await runTransaction(firestore, async (transaction: Transaction) => {
      ...
      if (order.type === "sell") {
        updatedUserData = {
          ...updatedUserData,
          [order.resource]: (Number(userDataDoc[order.resource]) || 0) + currentOrderAmount,
        };
      } else if (order.type === "buy") {
        updatedUserData = {
          ...updatedUserData,
          ufos: userDataDoc.ufos + (currentOrderAmount * order.pricePerUnit),
        };
      }
      ...
      transaction.update(doc(firestore, "UFOSperWallet", userDoc.id), {
        UFOS: updatedUserData.ufos,
        ...(order.type === "sell" && {
          [getFirestoreFieldFromUserDataKey(order.resource)]: updatedUserData[order.resource],
        }),
      });
